    ...
    for event in events:
        ...
        if d['rect']:
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                init_pos = pos
            if event.type == pg.MOUSEBUTTONUP:
                triangle(screen,[init_pos, pos,((pos[0] - init_pos[0])*cos(pi/3) - (pos[1] - init_pos[1])*sin(pi/3) + init_pos[0], (pos[0] - init_pos[0])*sin(pi/3) + (pos[1] - init_pos[1])*cos(pi/3) + init_pos[1])], w, color)
